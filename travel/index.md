...
<div class="sq-grid">
  ...
  <article class="sq-card">
    <a class="sq-link" href="{{ '/travel/mexico/' | relative_url }}">
      <img class="sq-thumb" src="{{ '/assets/img/travel/mexico/thumbs/Mama.jpg' | relative_url }}" alt="Mexico">
    </a>
    <div class="sq-meta">
  ...
  <article class="sq-card">
    <a class="sq-link" href="{{ '/travel/turkey/' | relative_url }}">
      <img class="sq-thumb" src="{{ '/assets/img/travel/turkey/thumbs/carpet1' | relative_url }}" alt="Turkey">
    </a>
    <div class="sq-meta">
...
<div id="atlas-wrap" style="margin-top:1rem;">
  {% include world.svg %}
</div>
...
#atlas-wrap path.visited{ /* class-based fallback */
  fill: rgba(176,0,32,.35) !important;
  stroke: rgba(176,0,32,.75) !important;
}
#atlas-wrap path.visited:hover{ fill: rgba(176,0,32,.55) !important; cursor:pointer; }
</style>

<script>
(function(){
  // Build a map iso2 -> url from _data/visited_countries.yml
  const visited = {
  ...
  function tryFindElement(svgRoot, iso) {
    // Try variants: lowercase id, uppercase id, exact iso, 'country-iso', ending-with-iso
    const variants = [ iso.toLowerCase(), iso.toUpperCase(), iso, 'country-' + iso.toLowerCase() ];
    for (const v of variants) {
      let el = svgRoot.querySelector('#' + CSS.escape(v));
      if (el) return el;
      // sometimes IDs use prefix or suffix, try attribute match
      el = svgRoot.querySelector('[id$="' + v + '"]');
      if (el) return el;
    }
    return null;
  }
  ...
  Object.keys(visited).forEach(function(iso){
    ...
    if (!el) return;
    // add a class (CSS fallback)
    el.classList.add('visited');
    // also enforce inline styles to override internal SVG styling if needed
    try {
      el.style.fill = 'rgba(176,0,32,0.35)';
      el.style.stroke = 'rgba(176,0,32,0.75)';
    } catch(e) { /* ignore style set issues */ }

    // click -> go to country page
    el.addEventListener('click', function(){
      window.location.href = visited[iso];
    });

    // optional: tooltip on hover (native title)
