...
<!-- GRID -->
<div class="sq-grid">
{% assign items = site.data["travel-mongolia"] | sort: "date" | reverse %}
{% for p in items %}
  <article class="sq-card" data-place="{{ p.place | escape }}">
    <a href="{{ '/assets/img/travel/mongolia/full/' | append: p.file | relative_url }}" class="sq-a">
      <img class="sq-thumb" src="{{ '/assets/img/travel/mongolia/thumbs/' | append: p.file | relative_url }}" alt="{{ p.name | escape }}">
    </a>
    <div class="sq-meta">
...
<div id="mongolia-map" style="height:420px;border-radius:14px;overflow:hidden;border:1px solid rgba(0,0,0,.12);"></div>

<script>
(function(){
  const raw = [
    {% for p in site.data["travel-mongolia"] %}
      {% if p.lat and p.lon %}
  ...
  const map = L.map('mongolia-map', { scrollWheelZoom:false });
